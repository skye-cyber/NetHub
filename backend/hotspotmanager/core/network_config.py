...
class NetworkConfigurator:
    ...
    def setup_bridge_configuration(self):
        ...
        try:
            ...
            if not self.ap_manager.is_bridge_interface(self.config['internet_iface']):
                ...
                command.run([
                    'ip', 'link', 'set', 'dev', self.config['internet_iface'], 'promisc', 'on'
                ], check=True)
    ...
    def configure_dns(self):
        ...
        try:
            ...
            command.run([
                'iptables', '-w', '-A', 'INPUT',
                '-p', 'tcp', '-m', 'tcp', '--dport', str(self.config['dns_port']),
                '-j', 'ACCEPT'
            ], check=True)

            command.run([
                'iptables', '-w', '-A', 'INPUT',
                '-p', 'udp', '-m', 'udp', '--dport', str(self.config['dns_port']),
                '-j', 'ACCEPT'
            ], check=True)
            ...
            command.run([
                ...
                '-j', 'REDIRECT', '--to-ports', str(self.config['dns_port'])
            ], check=True)
    ...
    def cleanup_bridge(self):
        ...
        try:
            ...
            if not self.ap_manager.is_bridge_interface(self.config['internet_iface']):
                ...
                command.run([
                    'ip', 'link', 'set', 'dev', self.config['internet_iface'], 'promisc', 'off'
                ], check=False)

                command.run([
                    'ip', 'link', 'set', 'dev', self.config['internet_iface'], 'nomaster'
                ], check=False)

                command.run([
                    'ip', 'link', 'delete', self.config['bridge_iface'], 'type', 'bridge'
                ], check=False)
                ...
                for addr in self.config.get('ip_addrs', []):
                    ...
                    addr = addr.replace(f"{self.config['internet_iface']}", '').strip()
                    command.run([
                        'ip', 'addr', 'add', addr, 'dev', self.config['internet_iface']
                    ], check=False)
                ...
                for route in self.config.get('route_addrs', []):
                    if route and not route.startswith('default'):
                        command.run([
                            'ip', 'route', 'add', route, 'dev', self.config['internet_iface']
                        ], check=False)
                ...
                self.netmanager.networkmanager_rm_unmanaged_if_needed(self.config['internet_iface'])
            ...
            if not self.config['no_virt'] and self.config.get('vwifi_iface'):
                ...
                if self.config.get('new_macaddr'):
                    command.run([
                        ...
                        'address', self.config.get('old_macaddr')
                    ], check=False)
        ...
        try:
            ...
            if not found and self.config['internet_iface']:
                ...
                forwarding_file = os.path.join(
                    self.conf_dir,
                    f"{self.config['internet_iface']}_forwarding"
                )

                if os.path.exists(forwarding_file):
                    with open(forwarding_file, 'r') as src, \
                            open(f"/proc/sys/net/ipv4/conf/{self.config['internet_iface']}/forwarding", 'w') as dst:
