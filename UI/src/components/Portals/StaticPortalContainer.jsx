...
import { Toast } from '../Toasts/ToastUI';
...
export const StaticPortalContainer = () => {
    ...
    useEffect(() => {
        ...
        document.addEventListener('static-portal-show', handleShowPortal);
        document.addEventListener('static-portal-show-targeted', handleShowTargetedPortal);
        document.addEventListener('static-portal-close', handleClosePortal);
        document.addEventListener('static-portal-close-container', handleCloseContainer);

        return () => {
            document.removeEventListener('static-portal-show', handleShowPortal);
            document.removeEventListener('static-portal-show-targeted', handleShowTargetedPortal);
            document.removeEventListener('static-portal-close', handleClosePortal);
            document.removeEventListener('static-portal-close-container', handleCloseContainer);
        };
    }, []);
    ...
    return (
        <>
            ...
            {Array.from(targetedPortals.entries()).map(([containerId, containerPortals]) => {
                ...
                let portalRoot = containerElement.querySelector('.data-portal-root');
                if (!portalRoot) {
                    console.warn(`Container .data-portal-root not found in ${containerId}: Defaulting to self`);
