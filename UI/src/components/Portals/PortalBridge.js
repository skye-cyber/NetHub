export class StaticPortalBridge {
    constructor() {
    ...
    showComponentInTarget(componentType, containerId, props = {}, portal_prefix = '') {
        ...
        const event = new CustomEvent('static-portal-show-targeted', {
    ...
    showComponent(componentType, props = {}, portal_prefix = '') {
        ...
        const event = new CustomEvent('static-portal-show', {
    ...
    closeComponent(portalId, prefix = false) {
        const event = new CustomEvent('static-portal-close', {
    ...
    closeAllInContainer(containerId) {
        const event = new CustomEvent('static-portal-close-container', {
            detail: { containerId }
        });
        document.dispatchEvent(event);
    }
...
export class StreamingPortalBridge {
    ...
    createStreamingPortal(componentType, containerId, initialProps = {}, portal_prefix = '') {
        ...
        // Initial render
        const event = new CustomEvent('streaming-portal-create', {
            detail: {
                portalId,
                componentType,
                containerId,
    ...
    updateStreamingPortal(portalId, newProps) {
        ...
        const event = new CustomEvent('streaming-portal-update', {
    ...
    _appendToStreamingPortal(portalId, data) {
        console.log(data)
        const event = new CustomEvent('streaming-portal-append', {
    ...
    closeStreamingPortal(portalId, prefix = false) {
        const event = new CustomEvent('streaming-portal-close', {
    ...
    batchUpdate(updates) {
        const event = new CustomEvent('streaming-portal-batch-update', {
            detail: { updates }
        });
        document.dispatchEvent(event);
    }
...
export function ClosePrefixed() {
    for (let pid of ['user_message', 'ai_message']) {
        PortalBridge.closeComponent(pid, true)
