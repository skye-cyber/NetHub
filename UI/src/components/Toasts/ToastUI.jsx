...
import { StaticPortalBridge } from '../Portals/PortalBridge';
...
export const Toast = ({ type = 'success', message = 'Action succeeded', messageId, duration = null, autoDismiss = true, portal_id }) => {
    ...
    return (
        <div
            ...
            className="message-toast fixed top-0 right-0 z-50 transform transition-all duration-500 ease-in-out opacity-0 translate-x-full backdrop-blur-lg mb-2"
...
export const SleakToast = ({ message = 'Action completed', body = 'Action status toast' }) => {

    const [content, setContent] = useState({ message: message, body: body })
    const modalRef = useRef(null)
    const titleRef = useRef(null)

    // Function to show the modal
    const showSleakToastFeedback = useCallback((display_content = {}) => {
        setContent({...content, ...display_content })
    ...
    StateManager.set('showSleakToastFeedback', showSleakToastFeedback)

    useEffect(() => {
        document.addEventListener('show-sleak-toast-feedback', showSleakToastFeedback)
        return () => {
            document.removeEventListener('show-sleak-toast-feedback', showSleakToastFeedback)
        }
    })

    return (
        <div ref={modalRef} id="SleakToastModal" className="fixed top-6 left-1/2 transform -translate-x-1/2 z-50 opacity-0 pointer-events-none transition-all duration-500 ease-in-out">
            <div className="bg-gradient-to-br from-white to-gray-50 dark:from-slate-800 dark:to-slate-900 border border-gray-200/60 dark:border-slate-600/50 rounded-2xl shadow-2xl shadow-black/20 backdrop-blur-xl p-4 min-w-[280px]">
                ...
                <div className="flex items-center justify-center space-x-3">
                    ...
                    <div className="text-left">
                        <p ref={titleRef} id="action-message" className="font-semibold text-gray-900 dark:text-white text-sm">{content.message}</p>
                        <p id="action-body" className="text-xs text-gray-500 dark:text-gray-400 mt-0.5">{content.body}</p>
                    </div>
                </div>

                {/* Progress Bar */}
                <div id="action-progress" className="mt-3 h-0.5 bg-gray-200 dark:bg-slate-700 rounded-full overflow-hidden">
