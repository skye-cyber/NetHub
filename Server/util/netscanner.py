...
class NetScanner:
    def __init__(self, subnet="192.168.12.0/24", interface="ap0"):
        ...
        self.authenticated_devices = (
            Device.objects.all().values("mac_address").values_list()
        )
        self.setup_logging()
        ...
        try:
            ...
            try:
                for line in result.stdout.split("\n"):
                    if (
                        self.interface in line
                        and f"{self.subnet.rsplit('.', 1)}." in line
                    ):
                        subnet_parts = self.subnet.split(".")
                        # Parse: ? (192.168.12.100) at ab:cd:ef:12:34:56 [ether] on wlan0
                        ip_match = re.search(
                            rf"\(({subnet_parts[0]}\.{subnet_parts[1]}\.{subnet_parts[2]}\.\d+)\)",
                            line,
                        )
