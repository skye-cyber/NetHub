...
class ServiceProvider(models.Model):
    ...
    logo = models.ImageField(
        upload_to="service_providers/logos/", blank=True, null=True
    )
    cover_image = models.ImageField(
        upload_to="service_providers/covers/", blank=True, null=True
    )
    ...
    business_hours = models.JSONField(
        default=dict, help_text="JSON structure for business hours"
    )
...
class Service(models.Model):
    ...
    distance = models.CharField(
        max_length=50, blank=True, null=True, help_text="Display distance from user"
    )
    is_online = models.BooleanField(
        default=False, help_text="Service is available online"
    )
...
class Advertisement(models.Model):
    ...
    categories = models.ManyToManyField(
        ServiceCategory, blank=True, related_name="advertisements"
    )
    ...
    def is_active(self):
        ...
        return (
            ...
            and (self.max_clicks == 0 or self.total_clicks < self.max_clicks)
            and (
                self.max_impressions == 0
                or self.total_impressions < self.max_impressions
            )
        )
